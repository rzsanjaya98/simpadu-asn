      ...
      <div class="col-md-10">
         <div class="card">
            ...
            <div class="card-body">
               ...
               <form action="{{ url('/leave_request/create') }}" method="POST">
                  @csrf
                  <div class="row">
                     @if (Auth::user()->role_id === 1)
                        <div class="form-group row">
                           ...
                           <div class="col-lg-9">
                              <select class="form-control form-small nested" name="user_id">
                                 <option>- Choose Employee -</option>
                                 @foreach ($userlists as $userlist)
                                    <option value="{{ $userlist->id }}" @selected(old('user_id') == $userlist->id)>{{ $userlist->employee->nip.' - '.$userlist->name }}</option>
                                 @endforeach
                              </select>
                           </div>
                        </div>
                        ...
                        <div class="col-lg-9">
                           <select class="select" name="type_leave_id">
                              ...
                              @foreach ($type_leaves as $type_leave)
                                  <option value="{{ $type_leave->id }}" @selected(old('type_leave_id') == $type_leave->id)>{{ $type_leave->type_leave }}</option>
                              @endforeach
                           </select>
                        </div>
                     </div>
